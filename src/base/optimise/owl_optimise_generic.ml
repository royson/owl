...
module Make
  ...
  module Learning_Rate = struct
    ...
    let run = function
      ...
      | AdaptiveRev a       -> fun _ _ c -> Maths.(_f a / sqrt (c.(1) + _f 1e-32))
      | AdaDelay a          -> fun i _ c -> Maths.(sqrt (c.(0) / (_f (float_of_int i)))) 
      | DelayComp (_, v, _) -> fun _ _ c -> Maths.(_f v / sqrt (c.(0) + _f 1e-7)) 
    ...
    let update_ch typ g c = match typ with
      ...
      | Adam (_, b1, b2) ->
          ...
          [|m; v|]
      | DelayComp (_, _, m) -> [|Maths.((_f m * c.(0)) + (_f 1. - _f m) * g * g); c.(1)|]
      | _                   -> c
  ...
  let update_network_server ?state params weights gradients delay loss update save x_size =
    ...
    let state = match state with
      ...
      | None       -> (
          ...
          Checkpoint.(state.us <- Owl_utils.aarr_map (fun _ -> _f 0.) gradient);
          let _ = match params.learning_rate with
          | AdaptiveRev _ -> Checkpoint.(state.ch <- Owl_utils.aarr_map (fun _ -> [|_f 1.; _f 1.|]) gradient)
          | _ -> Checkpoint.(state.ch <- Owl_utils.aarr_map (fun _ -> [|_f 0.; _f 0.|]) gradient)
          in
    ...
    | AdaptiveRev _ -> 
      ...
      Checkpoint.(state.ch <- Owl_utils.aarr_map3 (fun g gb c ->
        let z = Maths.((c.(0) + (g * g)) + (_f 2. * g * gb)) in
        [|z; max z c.(1)|]
    ...
    | AdaDelay a ->
      ...
      Checkpoint.(state.ch <- Owl_utils.aarr_map2 (fun g c ->
        let i = _f (float_of_int state.current_batch) in
        let z = Maths.(c.(0) + ((i / (i + (_f (float_of_int delay)))) * (g * g))) in
        [|z; c.(1)|]
      ) gradient state.ch);

      (* calculate delayed learning rate. c_j *)
      ...
      let mod_l = Checkpoint.(
        Owl_utils.aarr_map (fun dl ->
          Maths.(_f a / 
                ((dl * sqrt ((_f (float_of_int state.current_batch)) 
                + (_f (float_of_int delay)))) 
                + _f 1e-32)
                )
        ) delay_l
      ) in
