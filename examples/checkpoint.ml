...
let train () =
  ...
  let chkpt state =
    ...
    if state.current_batch mod (state.batches - 1) = 0 then (
      ...
      let h = Plot.create "seq_mnist_MLP_Adagrad_0.05.png" in
      ...
      Plot.set_title h "Sequential MNist Adam 0.01";
  ...
  let params = Params.config
    ~batch:(Batch.Sample 100) ~learning_rate:(Learning_Rate.Adagrad 0.05)
    ~checkpoint:(Checkpoint.Custom chkpt) ~stopping:(Stopping.Const 1e-6) 0.5
  in
